<script type="text/javascript">
	...
	(function pollNotyfication() {
		$.getJSON('<?php echo site_url('global/dashboard/my_task_board'); ?>', function (response) {
			if (response.new_task == 1) {
				console.log("You have received a task, good sir. " + lastUpdate );

				myNoty = '';
				

				$('#myTask').prepend(myNoty);
			}
			
			setTimeout(pollNotyfication, 60000);
		})  
		.fail(function() {
			console.log("error" + lastUpdate );
	...
	<div class="row">
		<div class="col-md-8 col-xs-12">
			...
			<div class="box box-success">
				...
				<div id="myTask" class="box-body" style="height: 380px; ">
					<ul class="todo-list">
<?php 
	foreach($notification_list->result() as $row) {
?>
					<li style="border-left: 2px solid #00c0ef; padding: 5px 10px; margin-bottom: 4px;">
<?php 
	list($c_date, $c_time) = explode(' ', $row->created_time);
	$c_date = db_to_human($c_date);
?>
						<i class="fa fa-calendar" title="<?php echo $c_date . ' ' . $c_time; ?>"></i>
						<?php echo $row->agenda; ?>
						<span class="handle">
							<i class="fa fa-ellipsis-v"></i>
						</span>
						<!-- Emphasis label -->
						<span class=""><?php echo substr($row->note, 0, 120); ?></span>
						<!-- General tools such as edit or delete-->
						<div class="tools">
							<a href="<?php echo site_url($row->detail_link); ?>" title="<?php echo $row->note; ?>"><i class="fa fa-info"> </i></a>
						</div>
					</li>
<?php 
	}
...
?>
				...
				<div class="small-box bg-aqua">
					...
					<a href="<?php echo site_url('global/dashboard/my_task_list/surat_eksternal_masuk'); ?>" class="small-box-footer">List <i class="fa fa-arrow-circle-right"></i></a>
				</div>
			</div>
			<!-- ./col -->
