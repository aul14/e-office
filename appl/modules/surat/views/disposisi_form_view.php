...
<?php	
	...
	if(has_permission(7) || (get_user_id() == $disposisi->from_user_id) || (in_array(get_user_id(), $allowed_user))) {
?>
	<!-- Default box -->
	<div class="box box-primary">
		<div class="box-header with-border">
...
<?php 
		if(has_permission(1)) {
			$list = $this->disposisi_model->get_subordinates(1, 1, 2);
		}else {
		...
		if(has_permission(7) || get_user_id() == $disposisi->from_user_id) {
					...
					<div class="col-md-5">
						...
						<div class="form-group">
							<label for="instruksi_to_<?php echo $i; ?>" class="col-md-4 control-label" style="padding-top: 0;">
								Lampiran
<?php
				if(!has_permission(7) && $row->status == 0) {
?>
...
?>
					...
					<div class="col-md-7">
<?php 
				$this->load->view('diskusi', array('id' => $key, 'function_handle' => 'surat.disposisi_model.set_diskusi', 'script_handle' => $i, 'ref_id' => $disposisi->disposisi_id, 'diskusi' => $row->diskusi, 'active' => (!has_permission(7)) ? (($row->status == 0) && ($disposisi->status == 1)) : ($row->status == 1) ));
?>
					</div>
