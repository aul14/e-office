...
?>
					<tr>
						...
						<td></td>
						<td width="100"><a href="<?php echo ($row->function_ref_id != 3) ? site_url('surat/external/incoming/' . $row->surat_id) : site_url('surat/internal/sheet_view/' . $row->surat_id); ?>" target="_blank"> <?php echo strtoupper($row->jenis_agenda) . ' - ' . $row->agenda_id; ?></a></td>
						<td width="120">
		...
		if(isset($list_disposisi->distribusi)) {
			...
			if (isset($row->distribusi_tujuan)) {
				...
				if ($row->surat_from_ref == '2') {
					$distribusi_unit 		= $distribusi_tujuan[0]['unit'];
					$distribusi_unit_id 	= $distribusi_tujuan[0]['unit_id'];
					$distribusi_kode 		= (isset($distribusi_tujuan[0]['kode'])) ? $distribusi_tujuan[0]['kode'] : '________';
					$distribusi_jabatan 	= $distribusi_tujuan[0]['jabatan'];
					$distribusi_nama 		= $distribusi_tujuan[0]['nama'];
					$distribusi_direktorat 	= $distribusi_tujuan[0]['dir'];
				}else {
