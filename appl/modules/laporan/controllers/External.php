...
//		$this->output_data['notification_list'] = $list;
	...
	function incoming() {
		...
		$list = $this->laporan_model->get_my_list('surat_masuk_eksternal');
		$this->output_data['list'] = $list;
		$this->output_data['title'] = 'Surat Masuk External';
		
		$this->load->view('external_incoming', $this->output_data);
		$this->load->view('global/footer');
	}
	...
	function outgoing() {
		...
		$list = $this->laporan_model->get_my_list('surat_keluar_eksternal');
		$this->output_data['list'] = $list;
		$this->output_data['title'] = 'Surat Keluar External';
		
		$this->load->view('external_outgoing', $this->output_data);
		$this->load->view('global/footer');
	}
		...
		if ($_POST['month'] != '' && $_POST['year'] != '') {
	        ...
	        foreach ($query as $element) {
	        	...
	        	if ($element['function_ref_id'] == 1) {
	        		...
	        		$element['surat_no'] = ($element['surat_no'] == '{surat_no}') ? trim($element['kode_klasifikasi_arsip']) . '/_/RSUD-BLJ' : $element['surat_no'];
	        	}
	...
	// 		$result = $this->laporan_model->get_surat_list($function_ref_id, $month, $year);
	// 		$konsep = $result->row();
