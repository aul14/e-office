...
class Internal extends LX_Controller {
	...
	function index() {
		...
		$list = $this->laporan_model->get_my_list('surat_internal');
		$this->output_data['list'] = $list;
		$this->output_data['title'] = 'Surat Nota Dinas';
		
		$this->load->view('internal_view', $this->output_data);
		...
		if ($_POST['month'] != '' && $_POST['year'] != '') {
	        ...
	        foreach ($query as $element) {
	        	...
	        	if ($element['function_ref_id'] == 1) {
	        		...
	        		$element['surat_no'] = ($element['surat_no'] == '{surat_no}') ? trim($element['kode_klasifikasi_arsip']) . '/_/NOTA DINAS' : $element['surat_no'];
	        	}
